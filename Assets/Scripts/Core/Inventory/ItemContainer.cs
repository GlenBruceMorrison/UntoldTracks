...
namespace UntoldTracks.InventorySystem
{
    ...
    public class ItemContainer
    {
        ...
        #region constructors
        public ItemContainer(Inventory inventory, int index, ItemModel item=null, int count=0, int durability=0)
        {
        ...
        /// </returns>
        public bool CanGive(ItemModel item, int count=1)
        {
            if (IsEmpty())
            {
                return true;
        ...
        internal int FillAndReturnRemaining(ItemModel item, int count, bool modifyContainer = true)
        {
            if (IsNotEmpty() && !HasItem(item))
            {
                throw new ArgumentException($"This container contains {item.displayName}, trying to add {_item.displayName}");
            }
